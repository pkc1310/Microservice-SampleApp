function updateMultiplication() {
 $.ajax({
 url: "http://localhost:9001/api/multiplications/random"
 }).then(function(data) {
 // Cleans the form
 $("#attempt-form").find( "input[name='resultattempt']" ).val("");
$("#attempt-form").find( "input[name='useralias']").val("");
...
function updateResults(alias) {
 ...
 $.ajax({
 async: false,
 url: "http://localhost:9001/api/results?alias=" + alias,
...
$(document).ready(function() {
 ...
 var $form = $( this ),
 attempt = $form.find( "input[name='resultattempt']" ).val(),
 userAlias = $form.find( "input[name='useralias']" )
.val();
...
{factorA: a, factorB: b}, resultAttempt: attempt};
 ...
 $.ajax({
 url: 'http://localhost:9001/api/results',
 type: 'POST',
